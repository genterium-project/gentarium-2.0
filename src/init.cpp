...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "gentarium.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "gentariumd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GTMand zGTMmoney supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 27117, 17217));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GTM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GTM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, gentarium, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GTM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Gentarium specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-gtmstake=<n>", strprintf(_("Enable or disable staking functionality for GTMinputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgtmstake=<n>", strprintf(_("Enable or disable staking functionality for zGTMinputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:27117"));
    ...
    strUsage += HelpMessageOpt("-backupzgtm=<n>", strprintf(_("Enable automatic wallet backups triggered after each zGTMminting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zgtmbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zGTMbackup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizegentariumamount=<n>     " + strprintf(_("Keep N GTManonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Gentarium Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Gentarium is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Gentarium process is using the data directory.
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gentarium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Gentarium: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Gentarium specific: zerocoin and spork DB's
                ...
                // Gentarium: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZGTMMinted();
                        RecalculateZGTMSpent();
                    }
                    RecalculateGTMSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Gentarium: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzGTMWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zGTMWallet
        uiInterface.InitMessage(_("Syncing zGTMwallet..."));

        bool fEnableZGtmBackups = GetBoolArg("-backupzgtm", true);
        pwalletMain->setZGtmAutoBackups(fEnableZGtmBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zgtmTracker->Init();
...
//    }
//
//    nAnonymizeGentariumAmount = GetArg("-anonymizegentariumamount", 0);
//    if (nAnonymizeGentariumAmount > 999999) nAnonymizeGentariumAmount = 999999;
//    if (nAnonymizeGentariumAmount < 2) nAnonymizeGentariumAmount = 2;
    ...
    LogPrintf("Anonymize Gentarium Amount %d\n", nAnonymizeGentariumAmount);
    ...
    /* Denominations
       ...
       For example:
       1GTM+1000 == (.1GTM+100)*10
       10GTM+10000 == (1GTM+1000)*10
    */
