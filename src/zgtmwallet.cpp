...
CzGTMWallet::CzGTMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgtm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGTMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGTMSeed_deprecated()) {
                    LogPrintf("%s: Updated zGTMseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgtm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgtm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGTMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGTMCount(nCountLastUsed);
    else if (!walletdb.ReadZGTMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGTMWallet::Lock()
{
    seedMaster = 0;
}

void CzGTMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGTMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGTM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGTMWallet::LoadMintPoolFromDB()
{
...
void CzGTMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGTMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGTMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgtmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGTMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGTM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgtmTracker which also adds to database
    pwalletMain->zgtmTracker->Add(dMint, true);
...
void CzGTMWallet::SeedToZGTM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGTMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGTMWallet::GenerateDeterministicZGTM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGTMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGTMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
