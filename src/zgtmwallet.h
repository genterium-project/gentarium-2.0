...
#ifndef Gentarium_ZGTMWALLET_H
#define Gentarium_ZGTMWALLET_H
...
public:
    CzGTMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGTM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGTM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Gentarium_ZGTMWALLET_H
