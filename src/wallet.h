...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GTMat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGTMsend
enum ZerocoinSpendStatus {
    ZGTM_SPEND_OKAY = 0,                            // No error
    ZGTM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGTM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGTM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGTM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGTM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGTM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGTM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGTM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGTM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGTM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGTM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGTM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGTM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGTM_SPENT_USED_ZGTM= 14,                      // Coin has already been spend
    ZGTM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGTM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZGtmBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZGTMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzgtmAuto) const;
    ...
    */
    ...
    CzGTMWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGTMTracker> zgtmTracker;
    ...
    void setZWallet(CzGTMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zgtmTracker = std::unique_ptr<CzGTMTracker>(new CzGTMTracker(strWalletFile));
    }

    CzGTMWallet* getZWallet() { return zwalletMain; }
    ...
    void setZGtmAutoBackups(bool fEnabled)
    {
    ...
    /** zGTMreset */
    boost::signals2::signal<void()> NotifyzGTMReset;
