...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZGtmStake(spend));
    } else {
        ...
        CGtmStake* gtmInput = new CGtmStake();
        gtmInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(gtmInput);
    }
