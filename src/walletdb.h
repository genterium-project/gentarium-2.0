...
public:
    ...
    bool WriteZGTMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGTMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGTMSeed_deprecated(uint256& seed);
    bool EraseZGTMSeed();
    bool EraseZGTMSeed_deprecated();

    bool WriteZGTMCount(const uint32_t& nCount);
    bool ReadZGTMCount(uint32_t& nCount);
