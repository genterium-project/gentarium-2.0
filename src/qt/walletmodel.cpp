...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered gentarium address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 GTM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzGTMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzGTMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzGTMReset.connect(boost::bind(NotifyzGTMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzGTMReset.disconnect(boost::bind(NotifyzGTMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zgtmTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
