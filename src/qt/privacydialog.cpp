                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGTM ought to be enough for anybody." - Bill Gates, 2017
    ui->zGTMpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzGTMSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGTMpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGTM_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGTM is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zGTM, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGTM...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGTM in ") +
...
void PrivacyDialog::on_pushButtonSpendzGTM_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGTM is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zGTM, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zGTM
        sendzGTM();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGTM
    sendzGTM();
}

void PrivacyDialog::on_pushButtonZGtmControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZGtmControlDialog* zGtmControl = new ZGtmControlDialog(this);
    zGtmControl->setModel(walletModel);
    zGtmControl->exec();
}

void PrivacyDialog::setZGtmControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGtmSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGTM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Gentarium Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGTMpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGTM is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GTM </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGTM</b>";
    ...
    // use mints from zGTM selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGtmControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZGtmControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGTM require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zGTM"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZGTM_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zGTM require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zGTM"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGTM transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zGTM was spent successfully update the addressbook with the label
    ...
    // Clear zgtm selector in case it was used
    ZGtmControlDialog::setSelectedMints.clear();
    ui->labelzGtmSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGTM Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Gtm, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGTM Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgtmTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGTM </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGTM "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGTM "));
    ui->labelzGTMAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zGTM </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zGTM </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzGTMSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzGTM->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zGTM
        ui->pushButtonMintzGTM->setEnabled(false);
        ui->pushButtonMintzGTM->setToolTip(tr("zGTM is currently disabled due to maintenance."));

        // Spend zGTM
        ui->pushButtonSpendzGTM->setEnabled(false);
        ui->pushButtonSpendzGTM->setToolTip(tr("zGTM is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zGTM
        ui->pushButtonMintzGTM->setEnabled(true);
        ui->pushButtonMintzGTM->setToolTip(tr("PrivacyDialog", "Enter an amount of GTM to convert to zGTM", 0));

        // Spend zGTM
        ui->pushButtonSpendzGTM->setEnabled(true);
        ui->pushButtonSpendzGTM->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
