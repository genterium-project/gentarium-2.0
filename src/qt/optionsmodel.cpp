...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GTM);
    ...
    if (!settings.contains("nAnonymizeGentariumAmount"))
        settings.setValue("nAnonymizeGentariumAmount", 1000);

    nAnonymizeGentariumAmount = settings.value("nAnonymizeGentariumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGentariumAmount"))
        SoftSetArg("-anonymizegentariumamount", settings.value("nAnonymizeGentariumAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGentariumAmount:
            return QVariant(nAnonymizeGentariumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGentariumAmount:
            nAnonymizeGentariumAmount = value.toInt();
            settings.setValue("nAnonymizeGentariumAmount", nAnonymizeGentariumAmount);
            emit anonymizeGentariumAmountChanged(nAnonymizeGentariumAmount);
            break;
        case CoinControlFeatures:
