...
// Copyright (c) 2017-2018 The Gentarium developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zgtmcontroldialog.h"
#include "ui_zgtmcontroldialog.h"
...
std::set<std::string> ZGtmControlDialog::setSelectedMints;
std::set<CMintMeta> ZGtmControlDialog::setMints;

ZGtmControlDialog::ZGtmControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGtmControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGtmControlDialog::~ZGtmControlDialog()
{
    delete ui;
}

void ZGtmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGtmControlDialog::updateList()
{
...
void ZGtmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGtmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGtm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGtmControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGtmControlDialog::GetSelectedMints()
{
...
void ZGtmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
